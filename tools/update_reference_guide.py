...
def main():
    ...
    insert('intro', 'controls', [
        ('guide', 'Your first guide book', 'You receive this guide once when joining the world. A carried copy counts; a full inventory delays delivery until space is free. Death and reconnecting do not grant another automatically. Lost it? Use /guide for a replacement.'),
        ('recover_guide', 'Recover your guide book', 'Use /guide or /kncraft guide to put this book in an empty inventory slot. Every player can use it. A carried copy, including your offhand, prevents duplicates. If your inventory is full, free a slot and retry. Crafting a book with one paper also still works.'),
        ('journal_links', 'From journal to chapter', 'Open a journal record, then select Open in Guide to jump to the relevant KNCraft Guide Book chapter. All records stay independent, without rewards or recipe locks. Read at your own pace and return to the journal when ready.'),
    ...
    data = read(declaration); data['version'] = 14; data['name'] = 'KNCraft Guide Book'; data['subtitle'] = 'KNCraft Guide Book'; declaration.write_text(json.dumps(data, indent=2) + '\n')
